...
namespace ProjectZ.InGame.Screens
{
    public class MenuScreen : Screen
    {
        ...
        public MenuScreen(string screenId)
            : base(screenId) { }
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            _backgroundWidth =
                (int)Math.Ceiling(Game1.WindowWidth / (double)(32 * _scale) + 1) * 32 * _scale;

            _menuRectangle = new Rectangle(
                Game1.WindowWidth / 2 - _menuWidth * _scale / 2,
                Game1.WindowHeight / 2 - _menuHeight * _scale / 2,
                _menuWidth * _scale,
                _menuHeight * _scale
            );
            ...
            _posX =
                (int)Math.Ceiling(_menuRectangle.X / (float)_scale / 32) * 32
                - _menuRectangle.X / _scale;
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            // Back Button Text
            {
                ...
                if (
                    ...
                    backStr = ControlHandler
                        .ButtonDictionary[ControlHandler.CancelButton]
                        .Keys[0]
                        .ToString();
                if (
                    !ControlHandler.LastKeyboardDown
                    && ControlHandler.ButtonDictionary[ControlHandler.CancelButton].Buttons.Length
                        > 0
                )
                    backStr = ControlHandler.GetButtonName(
            ...
            // Select Button Text
            {
                ...
                if (
                    ControlHandler.LastKeyboardDown
                    && ControlHandler.ButtonDictionary[ControlHandler.ConfirmButton].Keys.Length > 0
                )
                    selectStr = ControlHandler
                        .ButtonDictionary[ControlHandler.ConfirmButton]
                        .Keys[0]
                        .ToString();
                if (
                    !ControlHandler.LastKeyboardDown
                    && ControlHandler.ButtonDictionary[ControlHandler.ConfirmButton].Buttons.Length
                        > 0
                )
                    selectStr = ControlHandler.GetButtonName(
                        ControlHandler.ButtonDictionary[ControlHandler.ConfirmButton].Buttons[0]
                    );
            ...
            if (Game1.UiPageManager.PageHasTooltips())
            {
                ...
                if (
                    ControlHandler.LastKeyboardDown
                    && ControlHandler.ButtonDictionary[CButtons.Y].Keys.Length > 0
                )
                    tooltipStr = ControlHandler.ButtonDictionary[CButtons.Y].Keys[0].ToString();
                if (
                    !ControlHandler.LastKeyboardDown
                    && ControlHandler.ButtonDictionary[CButtons.Y].Buttons.Length > 0
                )
                    tooltipStr = ControlHandler.GetButtonName(
                        ControlHandler.ButtonDictionary[CButtons.Y].Buttons[0]
                    );
        ...
        public override void DrawTop(SpriteBatch spriteBatch)
        {
            ...
            spriteBatch.Draw(
                ...
                new Rectangle(
                    -_posX * _scale,
                    _menuRectangle.Y - _topBar * _scale,
                    _backgroundWidth,
                    _topBar * _scale
                ),
                new Rectangle(0, 0, _backgroundWidth / _scale, _topBar),
                Color.White
            );
            // bottom
            spriteBatch.Draw(
                ...
                new Rectangle(
                    -_posX * _scale,
                    _menuRectangle.Bottom,
                    _backgroundWidth,
                    _bottomBar * _scale
                ),
                new Rectangle(0, 0, _backgroundWidth / _scale, _bottomBar),
                Color.White
            );
            ...
            spriteBatch.Draw(
                _sprBackground,
                new Rectangle(
                    _menuRectangle.X - _leftBar * _scale,
                    _menuRectangle.Y,
                    _leftBar * _scale,
                    _menuHeight * _scale
                ),
                new Rectangle(0, 0, _leftBar, _menuHeight),
                Color.White
            );
            // right
            spriteBatch.Draw(
                _sprBackground,
                new Rectangle(
                    _menuRectangle.Right,
                    _menuRectangle.Y,
                    _rightBar * _scale,
                    _menuHeight * _scale
                ),
                new Rectangle(0, 0, _rightBar, _menuHeight),
                Color.White
            );
