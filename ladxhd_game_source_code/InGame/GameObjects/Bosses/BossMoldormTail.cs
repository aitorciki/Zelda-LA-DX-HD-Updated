...
namespace ProjectZ.InGame.GameObjects.Bosses
{
    ...
    {
        public BossMoldormTail(Map.Map map, BossMoldorm nightmare)
            : base(map)
        {
            ...
            AddComponent(
                ...
                new HittableComponent(hittableBox, nightmare.OnHitTail)
            );
            AddComponent(
                BaseAnimationComponent.Index,
                new AnimationComponent(animator, sprite, new Vector2(-8, -8))
            );
            AddComponent(DrawComponent.Index, new DrawCSpriteComponent(sprite, Values.LayerBottom));
            AddComponent(
                PushableComponent.Index,
                new PushableComponent(new CBox(EntityPosition, -6, -6, 12, 12, 8), OnPush)
                {
                    RepelMultiplier = 2.15f,
                }
            );
        }
