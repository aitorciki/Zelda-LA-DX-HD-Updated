...
namespace ProjectZ.InGame.GameObjects.Dungeon
{
    public class ObjTowerBackground : GameObject
    {
        ...
        public ObjTowerBackground()
            : base("final_cloud") { }

        public ObjTowerBackground(Map.Map map, int posX, int posY)
            : base(map)
        {
        ...
        private void Update()
        {
            // move the clouds
            _cloudOffset +=
                (0.75f + MathF.Sin(_cloudOffset) * 0.25f) * 0.0125f * Game1.TimeMultiplier;
        }
        ...
        private void Draw(SpriteBatch spriteBatch)
        {
            ...
            spriteBatch.Draw(
                ...
                new Rectangle(
                    ...
                    (int)(
                        cameraRectangle.Width / MapManager.Camera.Scale
                        + MapManager.Camera.Scale * 2
                    ),
                    (int)(Map.MapHeight * Values.TileSize - (int)(_spawnPosition.Y + 80))
                ),
                _colorCloud
            );
            ...
            spriteBatch.Draw(
                ...
                new Rectangle(
                    ...
                    (int)(_spawnPosition.Y + 48)
                        - (int)(
                            cameraRectangle.Y / MapManager.Camera.Scale - MapManager.Camera.Scale
                        )
                ),
                _colorSky
            );
            ...
            var leftCloud =
                (int)
                    Math.Floor(
                        (cameraRectangle.X / MapManager.Camera.Scale)
                            / _clouds.SourceRectangle.Width
                    ) - 1;
            var rightCloud = (int)
                Math.Ceiling(
                    (cameraRectangle.Right / MapManager.Camera.Scale)
                        / _clouds.SourceRectangle.Width
                );
            ...
            for (var i = leftCloud; i < rightCloud; i++)
            {
                ...
                DrawHelper.DrawNormalized(
                    ...
                    _clouds,
                    new Vector2(
                        x + _cloudOffset % _clouds.SourceRectangle.Width,
                        _spawnPosition.Y + 48
                    ),
                    Color.White
                );
