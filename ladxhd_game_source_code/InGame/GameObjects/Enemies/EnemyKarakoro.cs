...
namespace ProjectZ.InGame.GameObjects.Enemies
{
    internal class EnemyKarakoro : GameObject
    {
            ...
            : base(map)
        {
            ...
            stateRotate.Trigger.Add(
                new AiTriggerRandomTime(() => _aiComponent.ChangeState("idle"), 500, 750)
            );
            var stateIdle = new AiState { Init = InitIdle };
            stateIdle.Trigger.Add(new AiTriggerRandomTime(EndIdle, 250, 500));
            var stateBall = new AiState(UpdateBall) { Init = InitBall };
            stateBall.Trigger.Add(
                new AiTriggerCountdown(3300, null, () => _aiComponent.ChangeState("shake"))
            );
            ...
            AddComponent(
                ...
                _carriableComponent = new CarriableComponent(
                    new CRectangle(EntityPosition, new Rectangle(-8, -15, 16, 16)),
                    CarryInit,
                    CarryUpdate,
                    CarryThrow
                )
            ...
            AddComponent(
                ...
                _boxCollision = new BoxCollisionComponent(
                    new CBox(EntityPosition, -8, -14, 16, 14, 8),
            ...
            AddComponent(
                DrawComponent.Index,
                _drawComponent = new BodyDrawComponent(_body, DrawSprite, Values.LayerPlayer)
            );
            AddComponent(
        ...
        private void InitIdle()
        {
            ...
            {
                var box = new Box(
                    EntityPosition.Position.X - 7,
                    EntityPosition.Position.Y - 12,
                    0,
                    14,
                    12,
                    8
        ...
        private void ShakeTick(double counter)
        {
            _sprite.DrawOffset.X =
                _initShakeSpriteOffsetX
                + (float)Math.Sin((ShakeTime - counter) / 1000 * (60 / 4f) * Math.PI) * 2;
        }
        ...
        {
            if (
                !_body.FieldRectangle.Contains(
                    new RectangleF(
                        newPosition.X + _body.OffsetX,
                        newPosition.Y + _body.OffsetY,
                        _body.Width,
                        _body.Height
                    )
                )
            ...
            if (!_inHole && percentage > 0.50f && _aiComponent.CurrentStateId == "ball")
            {
                ...
                foreach (var gameObjectHole in _holeList)
                {
                    var collisionComponent =
                        gameObjectHole.Components[CollisionComponent.Index] as CollisionComponent;
                    ...
                    if (gameObjectHole is ObjHole holeObject)
                    {
                        ...
                        _holeTargetPosition = new Vector2(
                            holeObject.Center.X,
                            holeObject.Center.Y + 8
                        );
