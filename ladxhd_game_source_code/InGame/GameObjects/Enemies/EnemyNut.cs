...
namespace ProjectZ.InGame.GameObjects.Enemies
{
    class EnemyNut : GameObject
    {
        ...
        public EnemyNut(Map.Map map, Vector3 position, Vector3 direction)
            : base(map)
        {
            ...
            AddComponent(
                ...
                _pushComponent = new PushableComponent(hitBox, OnPush)
            );
            AddComponent(
                DamageFieldComponent.Index,
                _damageField = new DamageFieldComponent(hitBox, HitType.Enemy, 2)
            );
            AddComponent(
                HittableComponent.Index,
                _hitComponent = new HittableComponent(hitBox, OnHit)
            );
                ...
                if (_wasHit)
                {
                    ...
                    )
                    {
                        ...
                        Map.Objects.SpawnObject(
                            new ObjDeathExplodeEffect(Map, posX, posY, 0, 0, true)
                        );
                    }
                    else
                    {
