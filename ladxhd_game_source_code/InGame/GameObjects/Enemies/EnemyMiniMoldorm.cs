...
namespace ProjectZ.InGame.GameObjects.Enemies
{
    internal class EnemyMiniMoldorm : GameObject
    {
        ...
        public EnemyMiniMoldorm(Map.Map map, int posX, int posY)
            : base(map, "miniMoldormHead0")
        {
            ...
            AddComponent(
                ...
                _pushComponent = new PushableComponent(_bodyComp.BodyBox, OnPush)
            );
            AddComponent(
        ...
        private void UpdateHeadSprite(Vector2 direction)
        {
            ...
            _sprite.SourceRectangle = sprite
                ? _spriteHead1.ScaledRectangle
                : _spriteHead0.ScaledRectangle;
        ...
        private void Draw(SpriteBatch spriteBatch)
        {
            ...
            var posTwo =
                _tailTwoPosition
                - new Vector2(partTwoRectangle.Width / 2f, partTwoRectangle.Height / 2f);
            ...
            var posOne =
                _tailOnePosition
                - new Vector2(partOneRectangle.Width / 2f, partOneRectangle.Height / 2f);
        ...
        private void UpdateTailPositions()
        {
            // first
            var goalPosition =
                new Vector2(EntityPosition.X, EntityPosition.Y - SpriteOffsetY)
        ...
        private void OnCollision(Values.BodyCollision collision)
        {
            ...
            if ((collision & Values.BodyCollision.Horizontal) != 0)
                _direction = (float)
                    Math.Atan2(
                        -_bodyComp.VelocityTarget.X * _directionChangeMultiplier,
                        _bodyComp.VelocityTarget.Y
                    );
            else if ((collision & Values.BodyCollision.Vertical) != 0)
                _direction = (float)
                    Math.Atan2(
                        _bodyComp.VelocityTarget.X,
                        -_bodyComp.VelocityTarget.Y * _directionChangeMultiplier
                    );
        ...
        private bool OnPush(Vector2 direction, PushableComponent.PushType type)
        {
            if (type == PushableComponent.PushType.Impact)
                _bodyComp.Velocity = new Vector3(
                    direction.X * 2.5f,
                    direction.Y * 2.5f,
                    _bodyComp.Velocity.Z
                );
        ...
        {
            // absorb the tail
            _tailOnePosition = Vector2.Lerp(
                _tailOnePosition,
                new Vector2(EntityPosition.X, EntityPosition.Y - SpriteOffsetY),
                0.15f * Game1.TimeMultiplier
            );
            _tailTwoPosition = Vector2.Lerp(
                _tailTwoPosition,
                _tailOnePosition,
                0.15f * Game1.TimeMultiplier
            );
            ...
            var fallAnimation = new ObjAnimator(
                ...
                (int)EntityPosition.Y - 5 - SpriteOffsetY,
                Values.LayerBottom,
                "Particles/fall",
                "idle",
                true
