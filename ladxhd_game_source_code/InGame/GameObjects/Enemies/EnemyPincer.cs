...
namespace ProjectZ.InGame.GameObjects.Enemies
{
    internal class EnemyPincer : GameObject
    {
        ...
        public EnemyPincer(Map.Map map, int posX, int posY)
            : base(map)
        {
        ...
        private void Draw(SpriteBatch spriteBatch)
        {
            ...
            if (_sprite.IsVisible && _aiComponent.CurrentStateId != "spawning")
                for (var i = 0; i < 3; i++)
                {
                    var position =
                        _spawnPosition
                        + (EntityPosition.Position - _spawnPosition) * (0.15f + (i / 2f) * 0.5f)
                        - new Vector2(4, 4);
        ...
        private void GetAttackDirection()
        {
            ...
            var degree =
                MathHelper.ToDegrees((float)Math.Atan2(-_direction.Y, -_direction.X)) + 360;
        ...
        )
        {
            ...
            if (
                hitType == HitType.MagicPowder && _powderWindow
                || _overHole && _damageState.CurrentLives <= 0
            )
                FakeHoleDeath();
