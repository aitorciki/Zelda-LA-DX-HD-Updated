...
namespace ProjectZ.InGame.GameObjects.Enemies
{
    internal class EnemyGopongaFlowerGiant : GameObject
    {
        ...
        public EnemyGopongaFlowerGiant()
            : base("giant goponga flower") { }

        public EnemyGopongaFlowerGiant(Map.Map map, int posX, int posY)
            : base(map)
        {
            ...
            AddComponent(
                ...
                _pushComponent = new PushableComponent(body.BodyBox, OnPush)
            );
            AddComponent(
                DrawComponent.Index,
                new BodyDrawComponent(body, sprite, Values.LayerPlayer) { WaterOutline = false }
            );
        }
            ...
            if (_animator.CurrentAnimation.Id == "idle")
            {
                ...
                if (playerDistance.Length() < 128)
                {
                    ...
                    Map.Objects.SpawnObject(
                        new EnemyFireball(Map, (int)EntityPosition.X, (int)EntityPosition.Y, 0.8f)
                    );
        ...
        )
        {
            ...
            if (ValidateHit(hitType, pieceOfPower))
            {
                if (
                    hitType != HitType.BowWow
                    && (hitType == HitType.MagicRod || damage >= _damageState.CurrentLives)
                )
                {
