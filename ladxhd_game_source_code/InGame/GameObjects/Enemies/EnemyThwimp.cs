...
namespace ProjectZ.InGame.GameObjects.Enemies
{
    internal class EnemyThwimp : GameObject
    {
        ...
        public EnemyThwimp(Map.Map map, int posX, int posY)
            : base(map)
        {
            ...
            stateWait.Trigger.Add(
                new AiTriggerCountdown(700, null, () => _aiComponent.ChangeState("return"))
            );
            var stateReturn = new AiState { Init = InitReturn };
            var stateReturned = new AiState { Init = InitReturned };
            stateReturned.Trigger.Add(
                new AiTriggerCountdown(550, null, () => _aiComponent.ChangeState("idle"))
            );
        ...
        private void OnCollision(Values.BodyCollision collision)
        {
            if (
                ...
                && _aiComponent.CurrentStateId == "fall"
            )
                _aiComponent.ChangeState("wait");
            else if (
                (collision & Values.BodyCollision.Top) != 0
                && _aiComponent.CurrentStateId == "return"
            )
