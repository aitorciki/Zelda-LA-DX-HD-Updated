...
namespace ProjectZ.InGame.GameObjects.Things
{
    class ObjQuicksand : GameObject
    {
        ...
        public ObjQuicksand(Map.Map map, int posX, int posY, float dirX, float dirY, int mode)
            : base(map)
        {
            ...
            foreach (var gameObject in _collidingObjects)
            {
                ...
                )
                {
                    if (gameObjectBody.AdditionalMovementVT == Vector2.Zero)
                        gameObjectBody.AdditionalMovementVT =
                            gameObjectBody.LastAdditionalMovementVT;

                    var distance = gameObjectBody.BodyBox.Box.Center - _collisionBox.Center;
                    var distanceMult = 2 - Math.Clamp(distance.Length() / 8f, 0, 2);
                    gameObjectBody.AdditionalMovementVT = Vector2.Lerp(
                        gameObjectBody.AdditionalMovementVT,
                        _direction,
                        0.125f * Game1.TimeMultiplier * distanceMult
                    );
                }
            }
        }
