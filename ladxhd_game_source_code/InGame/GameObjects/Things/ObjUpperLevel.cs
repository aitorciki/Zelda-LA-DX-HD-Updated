...
namespace ProjectZ.InGame.GameObjects.Things
{
    internal class ObjUpperLevel : GameObject
    {
        ...
        public ObjUpperLevel()
            : base("editor upper level") { }

        public ObjUpperLevel(Map.Map map, int posX, int posY, int level)
            : base(map)
        {
            Map.AddFieldState(
                posX / 16,
                posY / 16,
                level == 1 ? MapStates.FieldStates.Level1 : MapStates.FieldStates.Level2
            );
            IsDead = true;
        }
    }
