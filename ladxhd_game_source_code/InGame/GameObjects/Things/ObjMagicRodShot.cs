...
namespace ProjectZ.InGame.GameObjects.Things
{
    class ObjMagicRodShot : GameObject
    {
        ...
        public ObjMagicRodShot(Map.Map map, CPosition linkPos, Vector2 offsetpos, int direction)
            : base(map)
        {
            ...
            if (magicrod_cast2d)
                spawnPosition = new Vector3(
                    linkPos.X + offsetpos.X,
                    linkPos.Y + offsetpos.Y - linkPos.Z,
                    0
                );
            ...
            _sprite = new CSprite("magicRodShot", EntityPosition) { Color = Color.Transparent };

            var body = new BodyComponent(
                EntityPosition,
                -2 + (direction == 1 ? 2 : (direction == 3 ? -2 : 0)),
                -2,
                4,
                4,
                8
            )
            {
                VelocityTarget =
                    ...
                    * (
                        Game1.GameManager.PieceOfPowerIsActive ? magicrod_speed + 1 : magicrod_speed
                    ),
                CollisionTypes =
        ...
        private void Update()
        {
            ...
            if (_spawnCounter > SpawnTime)
            {
                if (_spawnCounter > magicrod_duration)
                    _sprite.Color =
                        Color.White
                        * (1 - Math.Clamp((_spawnCounter - magicrod_duration) / FadeOutTime, 0, 1));
                else
                    _sprite.Color =
                        Color.White * Math.Clamp((_spawnCounter - SpawnTime) / FadeInTime, 0, 1);
        ...
        private void OnCollision(Values.BodyCollision collision)
        {
            ...
            var animation = new ObjBurningEffect(
                Map,
                (int)EntityPosition.X,
                (int)EntityPosition.Y - (int)EntityPosition.Z,
                0,
                0
            );
