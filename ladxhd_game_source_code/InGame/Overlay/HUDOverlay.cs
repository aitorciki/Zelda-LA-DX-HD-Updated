...
namespace ProjectZ.InGame.Overlay
{
    public class HudOverlay
    {
        ...
        public HudOverlay()
        {
            ...
            {
                _heartBackground = new UiRectangle(
                    Rectangle.Empty,
                    "heart",
            ...
            {
                _rupeeBackground = new UiRectangle(
                    Rectangle.Empty,
                    "rupee",
                    Values.ScreenNameGame,
                    Values.OverlayBackgroundColor,
            ...
            if (custom_keys_show)
            {
                _keyBackground = new UiRectangle(
                    Rectangle.Empty,
                    "keys",
                    Values.ScreenNameGame,
                    Values.OverlayBackgroundColor,
        ...
        public void Update(float fadePercentage, float transparency)
        {
            ...
            if (_saveIconCounter < 0)
                _saveIconCounter = 0;
            _saveIconTransparency = Math.Min(
                Math.Clamp(_saveIconCounter / 100, 0, 1),
                Math.Clamp((SaveIconTime - _saveIconCounter) / 100, 0, 1)
            );

            var scale = Math.Min(
                Game1.WindowWidth / (float)Values.MinWidth,
                Game1.WindowHeight / (float)Values.MinHeight
            );
            ...
            _gameUiWindow.Width = MathHelper.Clamp(
                (int)(Game1.WindowHeight * ar),
                0,
                Game1.WindowWidth
            );
            _gameUiWindow.Height = MathHelper.Clamp(
                (int)(Game1.WindowWidth / ar),
                0,
                Game1.WindowHeight
            );
            ...
            if (custom_rupee_show)
            {
                _heartPosition = new Point(
                    _gameUiWindow.X + 16 * Game1.UiScale + custom_heart_offsetx,
                    _gameUiWindow.Y + 16 * Game1.UiScale + custom_heart_offsety
                );
                _heartBackground.Rectangle = ItemDrawHelper.GetHeartRectangle(
                    _heartPosition,
                    _heartScale
                );
                _heartBackground.Rectangle.X -= (int)(
                    fadePercentage * FadeOffsetBackground * _heartScale
                );
            ...
            if (custom_heart_show)
            {
                _rubeePosition = new Point(
                    _gameUiWindow.X
                        + _gameUiWindow.Width
                        - ItemDrawHelper.RubeeSize.X * _rupeeScale
                        - 16 * Game1.UiScale
                        + custom_rupee_offsetx,
                    _gameUiWindow.Y + 16 * Game1.UiScale + custom_rupee_offsety
                );
                _rupeeBackground.Rectangle = ItemDrawHelper.GetRubeeRectangle(
                    new Point(_rubeePosition.X, _rubeePosition.Y),
                    _rupeeScale
                );
                _rupeeBackground.Rectangle.X += (int)(
                    fadePercentage * FadeOffsetBackground * _rupeeScale
                );
            ...
            if (custom_keys_show)
            {
                _keyPosition = new Point(
                    _gameUiWindow.X
                        + _gameUiWindow.Width
                        - ItemDrawHelper.KeySize.X * _keysScale
                        - 16 * Game1.UiScale
                        + custom_keys_offsetx,
                    _gameUiWindow.Y + 16 * 2 * _rupeeScale + custom_keys_offsety
                );
                _keyBackground.Rectangle = ItemDrawHelper.GetKeyRectangle(
                    new Point(_keyPosition.X, _keyPosition.Y),
                    _keysScale
                );
                _keyBackground.Rectangle.X += (int)(
                    fadePercentage * FadeOffsetBackground * _keysScale
                );
            ...
            _itemSlotOverlay.UpdatePositions(
                _gameUiWindow,
                new Point(
                    direction * (int)(fadePercentage * FadeOffsetBackground * Game1.UiScale),
                    0
                ),
                _itemsScale
            );
            ...
            if (custom_sicon_show)
            {
                _saveIconPosition = new Vector2(
                    GameSettings.ItemsOnRight
                        ...
                        : _gameUiWindow.X
                            + _gameUiWindow.Width
                            - _saveIcon.SourceRectangle.Width * Game1.UiScale
                            - 16 * _siconScale,
                    _gameUiWindow.Y
                        + _gameUiWindow.Height
                        - _saveIcon.SourceRectangle.Height * Game1.UiScale
                        - 16 * _siconScale
                );
            }
        ...
        public void DrawTop(SpriteBatch spriteBatch, float fadePercentage, float transparency)
        {
            ...
            if (custom_items_show)
                ItemSlotOverlay.Draw(
                    spriteBatch,
                    _itemSlotOverlay.ItemSlotPosition
                        + new Point(
                            GameSettings.ItemsOnRight
                                ? 1
                                : -1 * (int)(fadePercentage * FadeOffset * _itemsScale),
                            0
                        ),
                    _itemsScale,
                    transparency
            ...
            // draw dungeon keys
            if (custom_keys_show)
                ItemDrawHelper.DrawSmallKeys(
                    spriteBatch,
                    _keyPosition + new Point((int)(fadePercentage * FadeOffset * _keysScale), 0),
                    _keysScale,
            ...
            // draw the rubees
            if (custom_rupee_show)
                ItemDrawHelper.DrawRubee(
                    spriteBatch,
                    _rubeePosition + new Point((int)(fadePercentage * FadeOffset * _rupeeScale), 0),
                    _rupeeScale,
            ...
            if (custom_heart_show)
                ItemDrawHelper.DrawHearts(
                    spriteBatch,
                    _heartPosition - new Point((int)(fadePercentage * FadeOffset * _heartScale), 0),
                    _heartScale,
                    Color.White * transparency
                );
        }
        ...
        public void DrawBlur(SpriteBatch spriteBatch)
        {
            ...
            {
                Resources
                    .RoundedCornerBlurEffect.Parameters["blurColor"]
                    .SetValue(
                        (Values.OverlayBackgroundBlurColor * _saveIconTransparency).ToVector4()
                    );
                DrawHelper.DrawNormalized(
                    spriteBatch,
                    _saveIcon.Texture,
                    _saveIconPosition,
                    _saveIcon.ScaledRectangle,
                    Values.OverlayBackgroundColor * _saveIconTransparency,
                    _siconScale
                );
            }
        }
