...
namespace ProjectZ.InGame.Pages
{
    class SwordInteractPage : InterfacePage
    {
        ...
        public void SetSwordCollectNormal(bool state) =>
            ((InterfaceToggle)_toggleSwordGrabNormal.Elements[1]).ToggleState = state;

        public void SetSwordCollectStatic(bool state) =>
            ((InterfaceToggle)_toggleSwordGrabStatic.Elements[1]).ToggleState = state;

        public void SetSwordCollectFairy(bool state) =>
            ((InterfaceToggle)_toggleSwordGrabFairy.Elements[1]).ToggleState = state;

        public void SetSwordCollectKeys(bool state) =>
            ((InterfaceToggle)_toggleSwordGrabKeys.Elements[1]).ToggleState = state;

        public void SetSwordBounceBoomerang(bool state) =>
            ((InterfaceToggle)_toggleBounceBoomerang.Elements[1]).ToggleState = state;

        public void SetSwordBounceBombs(bool state) =>
            ((InterfaceToggle)_toggleBounceBombs.Elements[1]).ToggleState = state;

        public void SetSwordBlockProjectile(bool state) =>
            ((InterfaceToggle)_toggleSwordBlock.Elements[1]).ToggleState = state;

        public void SetSwordSmashesPots(bool state) =>
            ((InterfaceToggle)_toggleSmashPots.Elements[1]).ToggleState = state;

        public void SetSwordBeamCutsShrubs(bool state) =>
            ((InterfaceToggle)_toggleBeamShrubs.Elements[1]).ToggleState = state;
        ...
        public SwordInteractPage(int width, int height)
        {
            ...
            _swordSettingsList = new InterfaceListLayout
            {
            ...
            _swordSettingsList.AddElement(
                new InterfaceLabel(
                    Resources.GameHeaderFont,
                    "settings_sword_header",
            ...
            _toggleSwordGrabNormal = InterfaceToggle.GetToggleButton(
                new Point(buttonWidth, buttonHeight),
                new Point(5, 2),
                "settings_sword_grabnormal",
                GameSettings.SwGrabNormal,
                newState =>
                {
                    GameSettings.SwGrabNormal = newState;
                }
            );
            _contentLayout.AddElement(_toggleSwordGrabNormal);

            // Toggle: Collect Static Items
            _toggleSwordGrabStatic = InterfaceToggle.GetToggleButton(
                new Point(buttonWidth, buttonHeight),
                new Point(5, 2),
                "settings_sword_grabworlditem",
                GameSettings.SwGrabWorldItem,
                newState =>
                {
                    GameSettings.SwGrabWorldItem = newState;
                }
            );
            _contentLayout.AddElement(_toggleSwordGrabStatic);

            // Toggle: Collect Fairies
            _toggleSwordGrabFairy = InterfaceToggle.GetToggleButton(
                new Point(buttonWidth, buttonHeight),
                new Point(5, 2),
                "settings_sword_grabfairy",
                GameSettings.SwGrabFairy,
                newState =>
                {
                    GameSettings.SwGrabFairy = newState;
                }
            );
            _contentLayout.AddElement(_toggleSwordGrabFairy);

            // Toggle: Collect Small Keys
            _toggleSwordGrabKeys = InterfaceToggle.GetToggleButton(
                new Point(buttonWidth, buttonHeight),
                new Point(5, 2),
                "settings_sword_grabsmallkeys",
                GameSettings.SwGrabSmallKey,
                newState =>
                {
                    GameSettings.SwGrabSmallKey = newState;
                }
            );
            _contentLayout.AddElement(_toggleSwordGrabKeys);

            // Toggle: Bounce Boomerang
            _toggleBounceBoomerang = InterfaceToggle.GetToggleButton(
                new Point(buttonWidth, buttonHeight),
                new Point(5, 2),
                "settings_sword_boomerang",
                GameSettings.SwBoomerang,
                newState =>
                {
                    GameSettings.SwBoomerang = newState;
                }
            );
            _contentLayout.AddElement(_toggleBounceBoomerang);

            // Toggle: Bounce Bombs
            _toggleBounceBombs = InterfaceToggle.GetToggleButton(
                new Point(buttonWidth, buttonHeight),
                new Point(5, 2),
                "settings_sword_bouncebombs",
                GameSettings.SwSmackBombs,
                newState =>
                {
                    GameSettings.SwSmackBombs = newState;
                }
            );
            _contentLayout.AddElement(_toggleBounceBombs);

            // Toggle: Sword Block Projectiles
            _toggleSwordBlock = InterfaceToggle.GetToggleButton(
                new Point(buttonWidth, buttonHeight),
                new Point(5, 2),
                "settings_sword_swordblock",
                GameSettings.SwMissileBlock,
                newState =>
                {
                    GameSettings.SwMissileBlock = newState;
                }
            );
            _contentLayout.AddElement(_toggleSwordBlock);

            // Toggle: Smash Pots & Skulls
            _toggleSmashPots = InterfaceToggle.GetToggleButton(
                new Point(buttonWidth, buttonHeight),
                new Point(5, 2),
                "settings_sword_breakpots",
                GameSettings.SwBreakPots,
                newState =>
                {
                    GameSettings.SwBreakPots = newState;
                }
            );
            _contentLayout.AddElement(_toggleSmashPots);

            // Toggle: Beam Cuts Grass & Bushes
            _toggleBeamShrubs = InterfaceToggle.GetToggleButton(
                new Point(buttonWidth, buttonHeight),
                new Point(5, 2),
                "settings_sword_beamshrubs",
                GameSettings.SwBeamShrubs,
                newState =>
                {
                    GameSettings.SwBeamShrubs = newState;
                }
            );
        ...
        private string GetOptionToolip()
        {
            ...
            switch (index)
            {
                case 0:
                {
                    tooltip = Game1.LanguageManager.GetString("tooltip_sword_grabnormal", "error");
                    break;
                }
                case 1:
                {
                    tooltip = Game1.LanguageManager.GetString(
                        "tooltip_sword_grabworlditem",
                        "error"
                    );
                    break;
                }
                case 2:
                {
                    tooltip = Game1.LanguageManager.GetString("tooltip_sword_grabfairy", "error");
                    break;
                }
                case 3:
                {
                    tooltip = Game1.LanguageManager.GetString(
                        "tooltip_sword_grabsmallkeys",
                        "error"
                    );
                    break;
                }
                case 4:
                {
                    tooltip = Game1.LanguageManager.GetString("tooltip_sword_boomerang", "error");
                    break;
                }
                case 5:
                {
                    tooltip = Game1.LanguageManager.GetString("tooltip_sword_bouncebombs", "error");
                    break;
                }
                case 6:
                {
                    tooltip = Game1.LanguageManager.GetString("tooltip_sword_swordblock", "error");
                    break;
                }
                case 7:
                {
                    tooltip = Game1.LanguageManager.GetString("tooltip_sword_breakpots", "error");
                    break;
                }
                case 8:
                {
                    tooltip = Game1.LanguageManager.GetString("tooltip_sword_beamshrubs", "error");
                    break;
                }
            }
