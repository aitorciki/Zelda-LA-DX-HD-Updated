...
namespace ProjectZ.InGame.Pages
{
    class CopyPage : InterfacePage
    {
        ...
        private InterfaceListLayout[] _toSaveSlot = new InterfaceListLayout[
            SaveStateManager.SaveCount
        ];
        ...
        public CopyPage(int width, int height)
        {
            ...
            _mainLayout.AddElement(
                new InterfaceLabel(
                    ...
                    "main_menu_copy_header",
                    new Point(width, (int)(height * Values.MenuHeaderSize)),
                    new Point(0, 0)
                )
            );

            var fromToLayout = new InterfaceListLayout()
            {
            ...
            {
                var fromSaveSlot = new InterfaceListLayout()
                {
                    Size = new Point(saveButtonRec.X, (int)(height * Values.MenuContentSize)),
                };
                fromSaveSlot.AddElement(new InterfaceLabel("main_menu_copy_from"));
                for (var i = 0; i < SaveStateManager.SaveCount; i++)
                {
                    var _saveButtonLayouts = new InterfaceGravityLayout
                    {
                    ...
                    var saveSlotNumber = new InterfaceLabel(
                        null,
                        new Point(numberWidth, 12),
                        Point.Zero
                    )
                    {
                        Gravity = InterfaceElement.Gravities.Left,
                        TextAlignment =
                            InterfaceElement.Gravities.Center | InterfaceElement.Gravities.Bottom,
                    };
                    ...
                    // name on the right
                    {
                        ...
                        var middle = new InterfaceListLayout
                        {
                            ...
                            Size = new Point(rightWidth, 30),
                        };

                        // name
                        middle.AddElement(
                            _saveFromNames[i] = new InterfaceLabel(
                                null,
                                new Point(rightWidth - 3, 12),
                                Point.Zero
                            )
                            {
                                Margin = new Point(1, 0),
                                TextAlignment =
                                    InterfaceElement.Gravities.Left
                                    | InterfaceElement.Gravities.Bottom,
                            }
                        );
                    ...
                    _fromSaveSlot[i] = new InterfaceListLayout
                    {
                        HorizontalMode = true,
                        Gravity = InterfaceElement.Gravities.Right,
                        AutoSize = true,
                ...
                fromToLayout.AddElement(
                    new InterfaceLabel("main_menu_copy_arrow")
                    {
                        Size = new Point(6, 7),
                ...
                var toSaveSlot = new InterfaceListLayout()
                {
                    Size = new Point(saveButtonRec.X, (int)(height * Values.MenuContentSize)),
                    Selectable = true,
                };
                ...
                for (var i = 0; i < SaveStateManager.SaveCount; i++)
                {
                    ...
                    // name on the right
                    {
                        ...
                        // name
                        middle.AddElement(
                            _saveToNames[i] = new InterfaceLabel(
                                null,
                                new Point(rightWidth - 3, 12),
                                Point.Zero
                            )
                    ...
                    _toSaveSlot[i] = new InterfaceListLayout
                    {
                        HorizontalMode = true,
                        Gravity = InterfaceElement.Gravities.Right,
                        AutoSize = true,
        ...
        public override void OnReturn(Dictionary<string, object> intent)
        {
            ...
            if (
                intent != null
                && intent.TryGetValue("copyFile", out var copyFile)
                && (bool)copyFile
            )
            {
        ...
        private void UpdateUi()
        {
            ...
            {
                var slotName =
                    SaveStateManager.SaveStates[i] == null
                        ? Game1.LanguageManager.GetString("main_menu_copy_empty", "error")
