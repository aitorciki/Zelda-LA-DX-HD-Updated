    ...
    internal class TileSelectionScreen
    {
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (Game1.GameManager.MapManager.CurrentMap.MapName != _currentMapFileName)
            {
                ...
                _camera.Location.X =
                    (int)(Game1.WindowWidth - _tileMap.SprTileset.Width * _camera.Scale) / 2;
                _camera.Location.Y =
                    (int)(Game1.WindowHeight - _tileMap.SprTileset.Height * _camera.Scale) / 2;
            }
            ...
            {
                _currentSelection =
                    (mousePosition.X - _camera.Location.X)
                        / (int)(_tileMap.TileSize * _camera.Scale)
                        % _tileMap.TileCountX
                    + (mousePosition.Y - _camera.Location.Y)
                        / (int)(_tileMap.TileSize * _camera.Scale)
                        * _tileMap.TileCountX;
            ...
            if (InputHandler.MouseLeftReleased() && _selecting)
            {
                ...
                SelectedTiles = new int[
                    Math.Abs(end % _tileMap.TileCountX - start % _tileMap.TileCountX) + 1,
                    end / _tileMap.TileCountX - start / _tileMap.TileCountX + 1
                ];

                for (var y = start / _tileMap.TileCountX; y <= end / _tileMap.TileCountX; y++)
                for (
                    var x = Math.Min(start % _tileMap.TileCountX, end % _tileMap.TileCountX);
                    x <= Math.Max(start % _tileMap.TileCountX, end % _tileMap.TileCountX);
                    x++
                )
                {
                    SelectedTiles[
                        x - Math.Min(start % _tileMap.TileCountX, end % _tileMap.TileCountX),
                        y - start / _tileMap.TileCountX
                    ] = x + y * _tileMap.TileCountX;
                }
            }
        }
        ...
        public void Draw(SpriteBatch spriteBatch, bool blurTileset)
        {
            ...
            spriteBatch.Draw(
                ...
                new Rectangle(
                    ...
                    (tileset.Height / _tileMap.TileSize) * 16
                ),
                Color.White
            );
            ...
            if (_selecting)
            {
                spriteBatch.Draw(
                    ...
                    new Rectangle(
                        Math.Min(
                            _selectionStart % _tileMap.TileCountX,
                            _selectionEnd % _tileMap.TileCountX
                        ) * _tileMap.TileSize,
                        Math.Min(
                            _selectionStart / _tileMap.TileCountX,
                            _selectionEnd / _tileMap.TileCountX
